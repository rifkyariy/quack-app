...
enum QuackIconName {
    ...
    var sfSymbol: String {
        ...
        case .parent:  return "person.crop.circle.fill"
...
struct QuackIcon: View {
    ...
    var strokeWidth: CGFloat = 1.8 // unused; kept for call-site compat
...
struct TabBar: View {
    ...
    var body: some View {
        if axis == .horizontal {
            HStack(spacing: 0) {
                ForEach(TabItem.allCases, id: \.self) { tab in horizontalCell(tab) }
            }
            .padding(.horizontal, 12)
            .padding(.top, 12)
            .padding(.bottom, 28)  // clears the home-indicator inset in portrait
            .background(Color.paper)
            .shadow(color: .ink.opacity(0.08), radius: 8, x: 0, y: -4)
        } else {
            VStack(spacing: 6) {
                Spacer()
                ForEach(TabItem.allCases, id: \.self) { tab in verticalCell(tab) }
                Spacer()
            }
            .padding(.vertical, 20)
            ...
            .background(Color.paper)
            .shadow(color: .ink.opacity(0.08), radius: 8, x: -4, y: 0)
        }
    }

    private func horizontalCell(_ tab: TabItem) -> some View {
        let on = active == tab
    ...
    private func verticalCell(_ tab: TabItem) -> some View {
        let on = active == tab
        return Button { active = tab } label: {
            VStack(spacing: 5) {
                QuackIcon(name: tab.icon, size: 22,
                          color: on ? .quackOrange : .inkMuted,
                          strokeWidth: on ? 2.2 : 1.8)
                Text(tab.label)
            ...
            .background(
                RoundedRectangle(cornerRadius: 14)
                    .fill(on ? Color.quackOrange.opacity(0.12) : Color.clear)
            )
        }
        .buttonStyle(TapPress())
    }
...
// MARK: - MainTabChrome
/// Wraps a tab's scroll content with the `TabBar`, placing it at the bottom in
/// portrait and as a left/right sidebar in landscape — tracking whichever edge
/// currently hosts the home indicator, so it follows the device's physical
/// rotation direction.
struct MainTabChrome<Content: View>: View {
    @Binding var active: TabItem
    ...
    var body: some View {
        if isLandscape {
            GeometryReader { geo in
                // Ignoring safe area on a leaf alone doesn't reach the true edge —
                // the HStack still lays out from the inset origin. So the whole row
                // ignores horizontal safe area, and only content() (on the notch /
                // Dynamic Island side) gets its protection manually restored.
                let sidebarTrailing = OrientationLock.homeIndicatorOnTrailing
                let insets = geo.safeAreaInsets
                HStack(spacing: 0) {
                    if !sidebarTrailing {
                        TabBar(active: $active, axis: .vertical)
                    }
                    content()
                        .frame(maxWidth: .infinity)
                        .padding(.leading, sidebarTrailing ? insets.leading : 0)
                        .padding(.trailing, sidebarTrailing ? 0 : insets.trailing)
                    if sidebarTrailing {
                        TabBar(active: $active, axis: .vertical)
                    }
                }
            ...
            ZStack(alignment: .bottom) {
                content()
                TabBar(active: $active, axis: .horizontal)
            }
...
struct StickerTile: View {
    ...
    var body: some View {
        Button {
            ...
            .modifier(PulsingBorderEffect(active: locked))
            .cardShadow()
